...
``skills-service`` docker image is hosted on [DockerHub](https://hub.docker.com/r/skilltree/skills-service) and can be stared like this: 

```bash
docker run --name skills-service -d -p 8080:8080 skilltree/skills-service:latest
```

then you can tail running container's logs:
```bash
